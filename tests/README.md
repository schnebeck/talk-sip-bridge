...
| `test_two_publishers.py` | How many participants one call can carry. Two tones play in the room and the caller's audio is searched for both. Run it twice: by default the second is 57 dB down, and with `BRIDGE_MIX_PARTICIPANTS=true` both arrive at full level |
