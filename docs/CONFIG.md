...
| `BRIDGE_STATE_DIR` | `/var/lib/fritzbox-talk-bridge` | Directory for small per-line state files (currently just whether a line's registration should be on), so a crash-triggered restart resumes automatically instead of silently leaving the line deregistered until someone notices. `deploy/fritzbox-talk-bridge.service` provisions this path via systemd's `StateDirectory=`. Empty disables persistence (registration always starts off, as before this existed). |
