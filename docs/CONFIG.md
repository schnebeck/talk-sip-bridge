...
| `BRIDGE_PHONE_PARTICIPANT` | `phone` | How the caller appears in the room. `phone`: a virtual session flagged as a phone, showing the number - but Talk's clients build no peer for a participant without audio, and their "waiting for someone" sound then repeats every 15s for as long as the call lasts (measured; the web client stops after three, Android does not). `audio`: the same session flagged as carrying audio, which makes clients look for a stream a virtual session cannot have. `none`: no virtual session at all. **Measured not to work** with Talk 24/25 clients: without it there is nothing to answer, the client refuses with "a call to yourself cannot be answered", and the caller rings until the gateway gives up. The phone participant is what makes a call answerable, not decoration. |
