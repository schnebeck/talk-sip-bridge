<!--
talk-sip-bridge - docs/CONFIG.md
Every configuration variable, one by one.
...
dials a number without asking for a conversation: the dialog creates a
call room at that moment, and later calls are placed from inside it with
"call participant", including to further numbers. Observed here: one
such room carried calls to two different numbers, another eighteen calls
to one. So the room is neither the caller nor the callee, it is that
dialling session, and its token cannot be known in advance to be mapped
to anything. Routing on the conversation is only meaningful for a
dialout started inside a conversation that already existed. The
Nextcloud user who pressed the button is not in the request at all.
...
| `BRIDGE_MIX_PARTICIPANTS` | `false` | Whether a caller hears everybody in the conversation or one person. A telephone call carries one audio stream, so hearing several means summing them (`bridge/mixer.py`): one subscription per participant, mixed to a single stream, with the caller's own audio never added rather than subtracted afterwards. Off by default because one subscription is what every call this bridge has carried, and the summing path has not run against a real telephone - only against `tests/hardware/test_two_publishers.py`, where two participants playing 440 Hz and 1150 Hz both arrive at the caller (-0.3 dB and 0.0 dB; with this off, the second is 57 dB down). It does **not** cancel acoustic echo: what a caller's loudspeaker emits and their microphone picks up again comes back as a new signal. |
| `BRIDGE_MAX_MIXED_SOURCES` | 6 | How many participants at once, when mixing is on. Each is its own subscription and its own decoder. A larger room is carried by those whose flags say they are speaking, then the rest in a fixed order. Never fewer than one, whatever this says. |
...
| `BRIDGE_SIP_SHARED_SECRET` | (empty) | Talk's own `sip_bridge_shared_secret` (`occ config:app:get spreed sip_bridge_shared_secret`). With it the bridge can ask Nextcloud to create a conversation for an incoming call, in which the caller is a real participant - see `docs/SIGNALING-API.md`, "Direct dial-in". Needs a line's `DIALIN_NUMBERS` as well; without either, every inbound call goes to the room its line names. |
| `BRIDGE_DIALIN_NUMBERS` | (empty) | Per-line. Which numbers reaching this line are the bridge's own, as `<what the INVITE says>=<the number Nextcloud has>`, comma separated - e.g. `**622=4930622,**623=4930623`. Both halves are needed because a gateway announces an internal extension, which is a phone number to nobody but itself, while Nextcloud matches `talk_phone_numbers` (`occ talk:phone-number:add`) as an exact string. A call to a mapped number is answered by the bridge into the conversation Nextcloud creates for it; a call to any other number rings as usual, so one line carries both and a line sharing its number with a person's phone simply maps nothing. |
| `BRIDGE_CONFERENCE_NUMBERS` | (empty) | Per-line. Numbers on this line that are a conference line rather than one person's, comma separated, as the INVITE announces them - e.g. `**900`. A call to one is answered and asked which conversation it wants: the caller keys in the meeting id (the conversation's token, all digits wherever SIP is configured) and a PIN if that conversation wants one. A number here must not also be in `BRIDGE_DIALIN_NUMBERS`, which is the other thing a number can be. |
| `BRIDGE_CONFERENCE_CALLERS` | (empty) | Per-line. Who may reach those numbers: a regular expression the caller's own number has to match **in full** (`re.fullmatch`). Empty admits everybody, which on a line that also rings a person's phone means every caller in the world reaches the dial-in prompt - so this is the safety property, not a refinement. `[*][*][0-9]+` admits the gateway's own extensions and leaves every external call ringing as before. An invalid expression stops the daemon at startup rather than admitting anyone. |
| `BRIDGE_IVR_PROMPT_WAV` | (empty) | Recording(s) asking for the meeting id, comma separated and played in turn - the second only reached by a caller who has keyed nothing in after five seconds, which is how a second language is offered. 16-bit WAV, any sample rate, mono or stereo. Empty plays two rising beeps instead, which nobody can act on without being told what they mean. `deploy/make-ivr-prompts.sh` builds a set from text. |
| `BRIDGE_IVR_PIN_PROMPT_WAV` | (empty) | The same for the PIN question, reached only for a conversation that wants one. Empty plays three short beeps. |
| `BRIDGE_IVR_FIRST_DIGIT_TIMEOUT` | 25 | Seconds a caller gets before their first key. They are reading a number off an email, and on a mobile client they first have to switch the speaker on and open the keypad - which is most of half a minute before a key can be pressed at all. |
| `BRIDGE_IVR_NEXT_DIGIT_TIMEOUT` | 6 | Seconds between keys before the entry counts as finished. Long enough to look back at the email mid-number, short enough that a caller who is done is not left listening to silence. Ending with `#` skips the wait. |
| `BRIDGE_DTMF_DEBUG` | `false` | Logs every key press at the point it is read - which of the three roads (RTP event, tone in the audio, SIP INFO) carried it, the RTP timestamp that decides whether it is a new press, and every press dropped as a repeat - and writes everything the far end sent to `<BRIDGE_STATE_DIR>/dtmf-<call id>.wav` when the call ends. The only way to tell "the detector missed it" from "it was never sent"; off in normal operation, since it records call audio. |
| `BRIDGE_AUDIO_REPORT_INTERVAL` | 15 | Seconds between the two lines that say how a call's audio is doing - packets, silence-filled, dropped and peak in one direction, frames and peak in the other. Per second they were four fifths of everything the daemon ever logged and buried the lines that say what happened; the numbers are added up rather than dropped, because "is anything arriving, and is any of it being thrown away" is the first question about a call that sounds wrong. 0 turns them off. |
| `BRIDGE_EMPTY_ROOM_GRACE` | 30 | Seconds a room that looks empty is given to fill up again before the phone call is ended. Saving a new microphone in Talk *leaves* the call and rejoins - measured on this deployment: the audio flag dropped, the client left twelve seconds later, and was back after another 7.1. Ending the call on the first empty room therefore cuts a live conversation. Too long only costs a caller some silence on a line that is busy anyway, and they can hang up; too short costs the call. An explicit end - "end meeting for everyone", or the phone participant hung up in Talk - does not wait. |
| `BRIDGE_SIGNALING_DEBUG` | `false` | Logs every signaling message this bridge does not act on, and the whole payload of every room event. A room in a call produces a steady stream of them - mute, unmute and nick changes from every client, each addressed to the phone - so this buries the journal and is off in normal operation. On when the question is which message did *not* arrive. |
| `BRIDGE_IVR_PROMPT_GAP` | 5 | Seconds a prompt is given to work before the next one in `BRIDGE_IVR_PROMPT_WAV` is played, i.e. before the next language. |
| `BRIDGE_PHONE_PARTICIPANT` | `phone` | How the caller appears in the room. `phone`: a virtual session flagged as a phone, showing the number. `audio`: the same session flagged as carrying audio - which a virtual session never can, so Talk's clients build a peer for it and wait forever, showing an hourglass on the tile for the whole call. `none`: no virtual session at all, **measured not to work** with Talk 24/25 clients - without it there is nothing to answer, the client refuses with "a call to yourself cannot be answered", and the caller rings until the gateway gives up. The phone participant is what makes a call answerable, not decoration. `audio` was once preferred because `phone` was thought to leave Talk's "waiting for someone" sound repeating for the length of the call; measured again, it does not. That sound is capped at four plays, starts only for somebody who joins an empty call, and is stopped by any arrival - including a phone that carries no audio. |
