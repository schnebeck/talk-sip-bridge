...
| `BRIDGE_SIP_SHARED_SECRET` | (empty) | Talk's own `sip_bridge_shared_secret` (`occ config:app:get spreed sip_bridge_shared_secret`). With it the bridge can ask Nextcloud to create a conversation for an incoming call, in which the caller is a real participant - see `docs/SIGNALING-API.md`, "Direct dial-in". Needs a line's `DIALIN_NUMBER` as well; without either, every inbound call goes to the room its line names. |
| `BRIDGE_LINE_<id>_DIALIN_NUMBER` | (empty) | The number this line answers, in the form Nextcloud has it in `talk_phone_numbers` (`occ talk:phone-number:add`). Needed because a gateway announces an internal extension in the INVITE, which is a phone number to nobody but itself. |
